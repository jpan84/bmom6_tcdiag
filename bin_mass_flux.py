...
dtstr = '*.h1i.000[6-8]-*.nc'#'*.h1i.0009-*-0*-*.nc' #'*.h1i.000[6-7]-*.nc'
...
hist_kw = dict(hemi='warm', xnm='SSTr', ynm='MSE850r', innerlat=ILAT, outerlat=OLAT, thevarf=area_f, xvarf=lambda x: x['SSTr'], yvarf=lambda x: x['MSE850r'], xbins=np.arange(-8, 4.51, 0.25), ybins=np.arange(-4e4, 2.51e4, 2.5e3))
...
def main():
   ...
   outds.to_netcdf('0302_test_mf_vars/%s_%s_%s_yy06-08_%02d-%02d%s.nc' % (VARO, hist_kw['xnm'], hist_kw['ynm'], ILAT, OLAT, hist_kw['hemi']))
