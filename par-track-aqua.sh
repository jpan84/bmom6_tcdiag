...
FILES=$(ls "${PATHTOFILES}"/*.h1.*.nc | grep -v "$ignoreyear-")
###FILES=$(ls "${PATHTOFILES}"/*.h1.*regrid.nc | grep -v "$ignoreyear-")
###echo $FILES
for f in $FILES
do
...
STR_DETECT="--verbosity 0 --timestride 2 ${CONNECTFLAG} --out cyclones_tempest.${DATESTRING} --closedcontourcmd PSL,${DCU_PSLFOMAG},${DCU_PSLFODIST},0;_DIFF(Z300,Z500),${DCU_WCFOMAG},${DCU_WCFODIST},${DCU_WCMAXOFFSET} --mergedist ${DCU_MERGEDIST} --searchbymin PSL --outputcmd PSL,min,0;_VECMAG(UBOT,VBOT),max,2"
...
pwd
${TEMPESTEXTREMESDIR}/bin/StitchNodes --format "i,j,lon,lat,slp,wind" --allow_repeated_times --range ${SN_TRAJRANGE} --minlength ${SN_TRAJMINLENGTH} --maxgap ${SN_TRAJMAXGAP} --in cyclones.${DATESTRING} --out ${TRAJFILENAME} --threshold "wind,>=,${SN_MINWIND},${SN_MINLEN};lat,<=,${SN_MAXLAT},${SN_MINLEN};lat,>=,-${SN_MAXLAT},${SN_MINLEN}"
