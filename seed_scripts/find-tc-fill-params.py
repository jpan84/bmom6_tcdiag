...
import time
import subprocess
TID = str(time.perf_counter()).split('.')[0]
#DOUT = './debug_bmom/'
...
nl_base = os.path.basename(pthi)
DOUT = './debug_bmom_%s' % nl_base
if not os.path.exists(DOUT):
    os.makedirs(DOUT)
inic_file = args.inic_file

# !DEBUG
print('Debugging restart file %s at time %s' % (inic_file, TID))
...
if restart_file:
    ps = f['PSDRY'].values
    corrMass = np.sum(f['dpQ'].values, axis=1) + np.sum(f['dpCLDICE'].values, axis=1) + np.sum(f['dpCLDLIQ'].values, axis=1)
    ps += corrMass
    print(ps.shape)
    mps = ps[0, :]
    T = f['T'][0, :, :].values
...
print(gcdist.shape, mps.shape)
tmpps = np.where(gcdist > truePS_scan_radius, np.nan, mps)
print('Number of points within radius: %d' % int(~np.isnan(tmpps).sum()))
...
plt.savefig(os.path.join(DOUT, '%s.png' % TID))
