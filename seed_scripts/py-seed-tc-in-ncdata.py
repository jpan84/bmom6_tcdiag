...
if not invert_vortex:
    #!SZNL
    if not np.sign(sstmaxlat) == np.sign(cen_lat):
        raise ValueError("Vortex is not in the same hemisphere as SST max for seeding.")
    ...
    if not np.sign(sstmaxlat) == np.sign(cen_lat):
        print("Vortex %.2f° not in same hemisphere as SST max %.2f°...exiting" % (cen_lat, sstmaxlat))
        exit()
...
if not invert_vortex:
    ...
    rp = get_rp_from_dp_rmw(cen_lat, dp, target_rmw, debug=True)
    print(f"rp: {rp}  -- dp: {dp}")
