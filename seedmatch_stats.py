...
def main():
   ...
   mkplt = False
   clat, clon, psamb, tid, sstlat, sstval, dtstr, dp, rp, minsep, zp, exppr = tuple([np.nan for _ in range(12)])
   prevln = ''
   for ln in seedlog:
      spl = ln.strip('\n').split(' ')
      if not len(spl[0]):
      ...
      if mkplt:
         newrow = [dt.strptime('-'.join(dtstr.split('-')[:-1]), '%Y-%m-%d'), sstlat, sstval, tid, clat, clon, minsep, psamb, dp, rp, zp, exppr]
         #print(outdf.columns)
         #print(newrow)
