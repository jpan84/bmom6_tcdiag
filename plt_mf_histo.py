...
FILI = 'umf500_0010_histo_mse850_cape.pkl'
DIRO = 'mf_histo'

RAWVLIM = dict(vmin=0, vmax=6e10)
DIFCASEVLIM = dict(vmin=-1e10, vmax=1e10)
DIFSZNVLIM = dict(vmin=-2e10, vmax=2e10)
...
for ii, al in enumerate(ALIS):
   for jj, hm in enumerate(HEM):
      plt.pcolormesh(MSE850, CAPE, histos[ii * 2 + jj][0].T, shading='flat', cmap='viridis', **RAWVLIM)
      plt.xlim(*XLIM)
      plt.ylim(*YLIM)
...
for ii, al in enumerate(ALIS):
   ...
   plt.pcolormesh(MSE850, CAPE, dif, shading='flat', cmap='bwr', **DIFSZNVLIM)
   plt.xlim(*XLIM)
   plt.ylim(*YLIM)
   plt.xlabel('MSE 850 [J kg$^{-1}$]')
   plt.ylabel('CAPE [J kg$^{-1}$]')
...
for ii, al in enumerate(ALIS):
   for jj, hm in enumerate(HEM):
      ...
      plt.pcolormesh(MSE850, CAPE, dif, shading='flat', cmap='bwr', **DIFCASEVLIM)
      plt.xlim(*XLIM)
      plt.ylim(*YLIM)
      plt.xlabel('MSE 850 [J kg$^{-1}$]')
      plt.ylabel('CAPE [J kg$^{-1}$]')
