...
def main():
   ...
   cb = plt.colorbar(CSF, cax=axes[0, 1])
   cbt = cb.get_ticks()
   ...
   sumbud = integ3d + budds['SHFLX'] #sfcexch
   latwgts = np.cos(np.deg2rad(sumbud.lat))
   glob3d = (integ3d * latwgts).sum() / latwgts.sum()
   globbud = (sumbud * latwgts).sum() / latwgts.sum()

   ax = axes[1, 0]
   ax.plot(np.sin(np.deg2rad(sfcexch.lat)), sfcexch.values, label='sfc')
   ax.plot(np.sin(np.deg2rad(integ3d.lat)), integ3d.values, label='3D cond+lw+sw=%.2f W m$^{-2}$' % glob3d.values)
   ax.plot(np.sin(np.deg2rad(sumbud.lat)), sumbud.values, label='3D+SH=%.2f W m$^{-2}$' % globbud.values, color='gray')
