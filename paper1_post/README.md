...
### **Preprocessing requirements**
* **PP1**: run `par-track_driver.py` and `trajSN_to_df.py` to obtain TC trajectories
* **PP1A**: run all scripts in under PP1A to obtain the seasonally aggregated TC activity table (csv) from `track_dens.py`

### **Execution Instructions**
Run the script after setting the global var `FILI` to point to the csv from `track_dens.py`
