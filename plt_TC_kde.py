...
yarea = 2 * np.pi * 6.371e6**2 / 1e6 / 1e3**2 * np.diff(yinter)
...
zvals = [ko.evaluate(np.vstack([xgr.ravel(), ygr.ravel()])).reshape(xgr.shape) * nstms[ii] / NYR / yarea for ii, ko in enumerate(kdeobjs)]
print(zvals[0])
