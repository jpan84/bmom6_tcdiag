...
def main():
   ...
   ahu = wtm(derive_flx(camds, AHU))
   ahu_int = cam_glob_integ(ahu, camds.uxgrid.face_areas)
   plot_cam_zm(ahu, 'AHU: %.2f W m$^{-2}$' % ahu_int.values, 'ahu.png')

   restom = wtm(derive_flx(camds, RESTOM))
   restom_int = cam_glob_integ(restom, camds.uxgrid.face_areas)
   plot_cam_zm(restom, 'RESTOM: %.2f W m$^{-2}$' % restom_int.values, 'restom.png')
