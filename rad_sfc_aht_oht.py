...
def main():
   ...
   plt.plot(np.sin(np.deg2rad(momlat.isel(xh=0))), ohu_mom_zm, label='MOM %.2f' % ohu_mom_int)
   plt.savefig('OHU.png', bbox_inches='tight')
   plt.close()

   print('OHT')
   oht = wtm(derive_flx(momds, OHT)).sum(dim='xh')
   plt.plot(np.sin(np.deg2rad(momlat.isel(xh=0))), oht)
   plt.savefig('OHT.png', bbox_inches='tight')
   plt.close()

   print('AOHT from RESTOM')
   aoht_restom = hfl2ht(wtm(restom).zonal_mean(lat=momlat.isel(xh=0)))
